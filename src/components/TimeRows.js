...
class TimeRows  extends Component {
  ...
  createAppt(){
    for (var i=0; i<this.props.appointments.length; i++){
      ...
      let alignedTime = new Date()
      let hour = this.props.time.split(":")[0]
      let minutes = this.props.time.split(":")[1]
      alignedTime.setHours(hour, minutes)
      let matchTime = alignedTime.toLocaleString('en-US', { hour: 'numeric', hour12: true, minute: 'numeric'});
